...
class FilmController extends Controller
{
    ...
    {
        $films = Film::with('category', 'actors', 'user')->paginate(300);
/*dd($films->author_id);*/
    ...
    public function store(FilmRequest $request)
    {
        ...
        $film->toggleStatus($request->get('statuss'));
    ...
    public function edit(string $id)
    {
        ...
        if($film->author_id != \Auth::user()->id && \Auth::user()->is_admin != 1) {
            return redirect()->back()->withErrors('Ви не можете редагувати даний пост');
        }
    ...
    public function destroy(string $id)
    {
        ...
        if($film->author_id != \Auth::user()->id) {
            return redirect()->route('homee')->withErrors('Ви не можете видалити даний пост');
        }
