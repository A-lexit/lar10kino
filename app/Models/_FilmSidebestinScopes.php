...
//////////////////////////////////////////////////////////////////////////////
    ...
    {
        if($this->category->id === FILMSS_ID)
        return $sidefilms = Film::join('states', function ($j) {
            /* @var JoinClause $j*/
            $j->on('films.id', '=', 'states.film_id');
        })->with('category', 'state')->where('likes', '>', 0)->where('category_id', 1)->orderBy('likes', 'desc')->limit(20)->get();  //orderBy('likes', 'desc')

elseif($this->category->id === SERIALSS_ID)
    return $sidefilms = Film::join('states', function ($j) {
            /* @var JoinClause $j*/
            $j->on('films.id', '=', 'states.film_id');
        })->with('category', 'state')->where('likes', '>', 0)->where('category_id', 2)->orderBy('likes', 'desc')->limit(20)->get();  //orderBy('likes', 'desc')


elseif($this->category->id === MULTSS_ID)
    return $sidefilms = Film::join('states', function ($j) {
            /* @var JoinClause $j*/
            $j->on('films.id', '=', 'states.film_id');
        })->with('category', 'state')->where('likes', '>', 0)->where('category_id', 3)->orderBy('likes', 'desc')->limit(20)->get();  //orderBy('likes', 'desc')


elseif($this->category->id === MULTSERIALSS_ID)
    return $sidefilms = Film::join('states', function ($j) {
            /* @var JoinClause $j*/
            $j->on('films.id', '=', 'states.film_id');
        })->with('category', 'state')->where('likes', '>', 0)->where('category_id', 4)->orderBy('likes', 'desc')->limit(20)->get();  //orderBy('likes', 'desc')
    }
//////////////////////////////////////////////////////////////////////////
//5 кращих фільмів за Лайками в сайдбарі (1фільми, 2серіали, 3мульти, 4мультсеріали)
