...
class Film extends Model
{
    ...
    protected $fillable = ['note', 'subject', 'body', 'likes', 'vviews', 'view', 'season_id','quality_id', 'title', 'slug', 'origin_title', 'description', 'category_id', 'thumbnail', 'director_id', 'year_id', 'company_id', 'duration', 'other_actor','rating_id', 'composer_id', 'status_id', 'age_id', 'watch_id', 'films_title', 'categories_title' ];
...
//Пагінація фільмів Дивитись ще (фільми, серіали, мульти, мультсеріали)
    public function scopeLastLimit($query, $numbers)
    {
        return $query->where('category_id', FILMSS_ID)->with('category', 'state')->orderBy('created_at', 'desc')->limit($numbers)->get();  //orderBy('id', 'desc')
    }

    public function scopeLastLimit2($query, $numbers)
    {
        return $query->where('category_id', SERIALSS_ID)->with('category', 'state')->orderBy('created_at', 'desc')->limit($numbers)->get();  //orderBy('id', 'desc')
    }

    public function scopeLastLimit3($query, $numbers)
    {
        return $query->where('category_id', MULTSS_ID)->with('category', 'state')->orderBy('created_at', 'desc')->limit($numbers)->get();  //orderBy('id', 'desc')
    }

    public function scopeLastLimit4($query, $numbers)
    {
        return $query->where('category_id', MULTSERIALSS_ID)->with('category', 'state')->orderBy('created_at', 'desc')->limit($numbers)->get();  //orderBy('id', 'desc')
    }
    ...
    {
        return $query->where('slug', $slug)->with(['category', 'state'])->first();  //orderBy('id', 'desc')
    }
